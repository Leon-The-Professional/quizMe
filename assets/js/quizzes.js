...
let formsGenQz = {
    ...
    answers: [
        ...
        "6.	Indicates where to display the response after submitting the form. In HTML 4, this is the name/keyword for a frame. In HTML5, it is a name/keyword for a browsing context (for example, tab, window, or iframe). The following keywords have special meanings: <p><strong>_self</strong> (default): Load into the same browsing context as the current one.</p> <p><strong>_blank</strong>: Load into a new unnamed browsing context.</p> <p><strong>_parent</strong>: Load into the parent browsing context of the current one. If no parent, behaves the same as _self.</p> <p><strong>_top</strong>: Load into the top-level browsing context (i.e., the browsing context that is an ancestor of the current one and has no parent). If no parent, behaves the same as _self.</p>",
...
let ArraysQuiz = {
    ...
    questions: [
        ...
        `3.	What does the APM <strong>.push()</strong> do? Is it destructive? What value is returned?`,
        `4.	What does the APM <strong>.pop()</strong> do? Is it destructive? What value is returned?`,
        `5.	What does the APM <strong>.shift()</strong> do? Is it destructive? What value is returned?`,
        `6.	What does the APM <strong>.unshift()</strong> do? Is it destructive? What value is returned?`,
...
let StringsQuiz = {
    ...
    questions: [
        ...
        `6.	What is the <strong>.toUpperCase()</strong> method? Is this destructive?`,
        `7.	What is the <strong>.toLowerCase()</strong> method? Is this destructive?`,
        `8.	What is the <strong>.trim()</strong> method? What are the two more specific versions of this method? Is this destructive?`,
        `9.	Are string methods able to be combined?`,
        `10. What is the <strong>.indexOf()</strong> method?`,
        `11. What is the <strong>.slice()</strong> method? Is this destructive?`,
        `12. What is the <strong>.replace()</strong> method? Is this destructive?`,
        `13. What is the <strong>.repeat()</strong> method? Is this destructive?`,
    ...
    answers: [
        ...
        `6.	The toUpperCase() method returns the calling string value converted to uppercase (the value will be converted to a string if it isn't one). Not destructive.`,
        `7.	The toLowerCase() method returns the calling string value converted to lower case. Not destructive.`,
        `8.	The trim() method removes whitespace from both ends of a string and returns a new string, without modifying the original string. Whitespace in this context is all the whitespace characters (space, tab, no-break space, etc.) and all the line terminator characters (LF, CR, etc.) .trimStart() and .trimEnd(). Not destructive.`,
        ...
        `11. The slice() method extracts a section of a string and returns it as a new string, without modifying the original string. .slice(startPoint, endPoint) animal.slice(0,2) = tig. If there is no second argrument, the remainder of the string will be removed. Not destructive.`,
        `12. The replace() method returns a new string with some or all matches of a pattern replaced by a replacement. The pattern can be a string or a RegExp, and the replacement can be a string or a function to be called for each match. If pattern is a string, only the first occurrence will be replaced. The original string is left unchanged. .replace(‘string to be replaced’, ‘string to replace with’) animal.replace(‘t’, ‘l’) = liger .replaceAll(‘’,’’). Not destructive.`,
        `13. The repeat() method constructs and returns a new string which contains the specified number of copies of the string on which it was called, concatenated together. .repeat(number of repetitions) animal.repeat(2) = tigertiger. Not destructive.`,
